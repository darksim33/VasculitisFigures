...
def create_all_parameters_plot(plot_df, include_control=True):
    """
    ...
    # Parameter display information
    parameter_info = {
        "adc": {"title": "ADC Values", "ylabel": "ADC Values\n(x10^-6 mm²/s)"},
        "fa": {"title": "FA Values", "ylabel": "FA Values\n(dimensionless)"},
        "asl": {"title": "ASL Values", "ylabel": "ASL Values\n(ml/100g/min)"},
        "t2star": {"title": "T2* Values", "ylabel": "T2* Values\n(ms)"},
    }
    ...
    for idx, parameter in enumerate(parameters):
        ...
        # Plot each group in each region
        for roi in ["cortex", "medulla"]:
            if include_control:
                # Plot the healthy group - scatter on left, violin on right
                healthy_data = plot_df[
                ...
                if len(healthy_data) > 0:
                    ...
                    create_scatter_plot(
                        ...
                        x_position=positions[roi]["healthy"],
                        color=colors["healthy"],
                        label="healthy" if roi == "cortex" and idx == 0 else None,
                        jitter_range=0.05,
                        x_offset=-0.05,
                    )
        ...
        for roi in ["cortex", "medulla"]:
            if include_control:
                ...
                if (
                    ...
                    # Perform statistical test
                    p_value, sig_marker = perform_significance_test(
                        significance_data[roi]["healthy"],
                        significance_data[roi]["disease"],
                    )
    ...
    print(
        f"Combined plot with all parameters created and saved in the '{output_dir}' directory as:"
    )
    print(f"- {png_path.name} (raster format)")
    print(f"- {svg_path.name} (vector format)")
